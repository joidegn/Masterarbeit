...
\begin{tabular}{c|llll}
  ...
  \hline
  \hline
    & \multicolumn{3}{c}{\textit{In-Sample}} \\
	BIC & 13 & 13 & 5 & 1.30 \\
	FPE & 15 & 15 & 5 & 1.23 \\
	BIC and $IC_{p2}$ & 4 & 13 & 5 & 1.30 \\
	FPE and $IC_{p2}$ & 4 & 15 & 5 & 1.23 \\
	$IC_{p2}$ & 4 & 4 & 10 & 1.21 \\
  \hline
  \hline
  & \multicolumn{3}{c}{\textit{Out-of-Sample}} \\ 
   	RMSE & 1 & 1 & 1 & 0.71 \\
   	RMSE and $IC_{p2}$ & 4 & 1 & 1 & 0.71 \\
  \hline
  \multicolumn{4}{l} {\rule{0pt}{8ex} \begin{minipage}{6cm}
...
Repeating the exercise but allowing for lags of the factors to enter the forecasting equation we get the results in table \ref{results dynamic factor model, in-sample}.\footnote{Essentially, table \ref{results static factor model, in-sample} uses the same models as are used in table \ref{results dynamic factor model, in-sample} while restricting $q=0$.} Since the data used for the exercise consists of time-series it makes sense that a dynamic factor model is better suited for prediction. In the case of the dynamic model the FPE criterion seems to select too many lags of $y_t$ which results in a higher RMSE. The $IC_{p2}$ criterion chooses $r=4$ for the factor equation. For the forecasting equation, however, it seems to be better to reduce the number of factors to $2$ if the factors are also lagged once in the forecasting equation. This can be seen in the row showing the results for the BIC and the $IC_{p2}$. The resulting RMSE is RMSE$=0.58$ which is slightly lower thatn $0.59$, the RMSE for $r=4$ factors in the forecasting equation. While the difference is small, this shows that it can be optimal to choose a different number of factors in the forecasting equation than in the factor equation.

In both the static and the dynamic factor model, choosing the number of static factors using the IC$_{p2}$ criterion has at least slightly outperformed the other methods in terms of forecasting accuracy.
...
\begin{tabular}{c|lllll}
 ...
 \hline
	BIC & 2 & 2 & 3 & 1 & 0.58 \\
	FPE & 10 & 10 & 10 & 2 & 0.82 \\
	BIC and $IC_{p2}$ & 4 & 2 & 3 & 1 & 0.58 \\
	FPE and $IC_{p2}$ & 4 & 10 & 10 & 2 & 0.82 \\
	$IC_{p2}$ & 4 & 4 & 3 & 1 & 0.59 \\
  \hline
  & \multicolumn{3}{c}{\textit{Out-of-Sample}} \\ 
   	RMSE & 7 & 7 & 1 & 2 & 0.55 \\
   	RMSE and $IC_{p2}$ & 4 & 2 & 1 & 1 & 0.54 \\
  \hline
\multicolumn{5}{l} {\rule{0pt}{6ex} \begin{minipage}{6cm}
...
Given this situations we can not cut off the values before the break and re-estimate the model using data on the period after the break as was proposed above because we have strong indications that there are at least $2$ periods in the data with structural breaks, each of which affects a different set of factor loadings. We could treat both periods as structural breaks in the sense that only data after both structural breaks is used. This, however, would leave us with $17$ observations of which we have to remove another $15$ for the frist step in the pseudo out-of-sample forecasts. 

I propose three methods to overcome this issue. As a first way we can remember that some of the data series are monthly series which have been merged into the quarterly data set, dropping two thirds of the observations. We could in principle cast the data back into the monthly form interpolating the missing data. While this method solves our problem in a technical sense\footnote{For the first step of the pseudo out-of-sample period there are only $8$ data points for estimation. For every quarterly step of the pseudo out-of-sample procedure there are an additional $4$ observations. Thus, the ratio of observations to predictors improves quickly.}, it has several drawbacks: firstly no additional information is added although it appears to the model as if there are more observations. At the same time the imputation adds additional uncertainty. Traditional standard error estimates do not account for this and resultingly they may be downward biased (cf. \citet[chapter~25]{gelman2006missing}. As a result it is likely that prediction based on this short data set performs badly in a forecasting sense. On the other hand, if predictions resulting from this shaved data set should perform better, it would constitute a strong indication that the structural breaks uncovered by the tests should not be ignored. In this case doing so clearly leads to a significant degradation of forecasting results.



Since the data set is very wide and we only have relatively few series undergoing a structural break around 2008 we can also drop the series which are affected and redo the predictions exercise from above based on this limited data set. For the purpose of this paper we are interested in the performance of factor models which ignore structural breaks versus factor models which take the structural breaks into account. In the following both models will be compared in terms of forecasting precision based on the reduced data set.

Figure \ref{breaks per period, reduced data set} shows that the sup LM statistic $\mathscr{S}_{i,T}$ reports one clear period where most structural breaks are centered for the reduced data set.


Reestimating TODO:: continue here!


\begin{figure}[htp]
\centering
\caption{Structural breaks per quarter, reduced data set}
\label{breaks per period, reduced data set}
